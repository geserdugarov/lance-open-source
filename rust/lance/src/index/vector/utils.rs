...
async fn sample_training_data(
    ...
    if let Some(fragment_ids) = fragment_ids {
        ...
        // Rows the consumer still needs. The fragment producer reads this to
        // size each prefetch round to the outstanding demand rather than blindly
        // fetching `2 * sample_size_hint` rows per round.
...
/// rather than a fixed `2 * sample_size_hint`, so a low-null column reads at
/// most the requested sample instead of twice it. Visited offsets are tracked in a
/// [`RoaringTreemap`] because sparse or all-null columns force the stream to
/// visit most selected rows before it can terminate, and a compressed bitmap
/// keeps that persistent state near `num_rows / 8` bytes instead of the
/// ~16 bytes/entry a hash set would cost.
fn sample_training_data_scan_from_fragments(
    ...
    let stream = stream::try_unfold(
        ...
        move |(
            ...
            // Cap the round at the consumer's outstanding demand so a low-null
            // column reads at most the requested sample, matching the
            // non-nullable path. Previously each round read `2 * sample_size_hint`.
    ...
    S: Stream<Item = Result<RecordBatch>> + Unpin,
{
    ...
    while num_non_null < sample_size_hint {
        ...
        // Publish the outstanding demand so a fragment-limited producer can size
        // its next prefetch round to it rather than over-fetching.
        ...
        // Cap the copy at what we still need so `values_buf` cannot grow past its
    ...
    // Signal the producer to stop; the loop is done regardless of why it exited.
    if let Some(still_needed) = &still_needed {
    ...
    S: Stream<Item = Result<RecordBatch>> + Unpin,
{
    ...
    while num_non_null < sample_size_hint {
        ...
        // Bound each batch to what we still need so the retained batches, and the
        // post-loop `concat_batches`, stay sized to the sample rather than to the
        // over-fetched source.
...
mod tests {
    ...
    /// Nullable FSL with fragment-limited sampling must still fill the requested
    /// sample size. This exercises the `still_needed`-sized prefetch and the
    /// `accumulate_fsl_values` cap that together replace the old
    /// `2 * sample_size_hint` over-fetch.
    #[tokio::test]
    async fn test_maybe_sample_training_data_fsl_nullable_fragment_limited() {
        ...
        // The output is capped exactly at the requested sample size, never the
        // over-fetched amount.
    ...
    /// All-null nullable FSL with fragment-limited sampling: the consumer can
    /// never reach `sample_size_hint`, so the producer keeps sampling until it
    /// has exhausted every selected row. It must terminate and return an empty
    /// FSL rather than loop forever, while the persistent seen-offset set stays a
    /// compressed `RoaringTreemap` instead of growing a hash set to O(rows).
    #[tokio::test]
    async fn test_maybe_sample_training_data_fsl_all_null_fragment_limited() {
        let nrows: usize = 2000;
        let dims: u32 = 8;
        let sample_size: usize = 500;

        // 100% nulls => no non-null rows to sample.
        ...
        let dataset = InsertBuilder::new("memory://fsl_sample_all_null_test")
            .execute(vec![data])
            .await
            .unwrap();
    ...
    /// over-fetch (e.g. the old `2 * sample_size_hint`, or a `still + still/8`
    /// over-read) that a post-truncation output-length check cannot.
    #[tokio::test]
    ...
    #[test]
    fn test_accumulate_fsl_values_respects_max_rows_without_nulls() {
        let dim: usize = 4;
        ...
        // Only `max_rows` rows are copied, and they are the leading rows.
    ...
    fn test_accumulate_fsl_values_respects_max_rows_with_filter() {
        ...
        // Nulls are dropped and the copy is still capped at `max_rows`.
        assert_eq!(num_rows, max_rows);
        assert_eq!(buf.len(), max_rows * byte_width);
    }
}
