...
| 107 | `IvfRq` | `"IVF_RQ"` | IVF + RaBitQ (default 5 bits per dimension) |
...
**Struct:** `IvfModel` — `rust/lance-index/src/vector/ivf/storage.rs`
...
For more than 256 centroids, `KMeansParams::hierarchical_k > 1` enables
hierarchical training (default branching factor 16). In v12, child centroid
quotas are proportional to the training rows assigned to each branch, with
the requested final centroid count preserved. Optional global Lloyd refinement
uses `refine_iters` (default 0), and `seed` can make initialization reproducible.
These are k-means-core controls, not additional `IvfBuildParams` fields;
training still runs inside the worker/coordinator process.
...
For a **prefiltered** HNSW query, `ApproxMode::Fast` selects Lance's ACORN-1
traversal. It searches the mask-passing subgraph and uses bounded bridge
expansion to reach otherwise disconnected passing regions. `Normal` remains
the default HNSW traversal; `approx_mode` also controls separate RQ trade-offs,
so benchmark recall and latency for the complete index configuration.
...
| Field | Default | Meaning |
|---|---:|---|
| `num_bits` | 5 | Any value in `1..=9`; 1 is binary and larger values enable the extended code. |
| `rotation_type` | `Fast` | `Fast` (FHT-KAC, O(D log D)) or `Matrix` (dense, O(D²)) |

The five-bit default applies across Rust, Python, and Java in v12. Request
`num_bits=1` explicitly for binary RaBitQ; existing segments retain the bit width
stored in their model metadata.

At the five-bit default, a 768-dimensional packed code is about 480 B before
metadata and alignment (one sign bit plus four extended bits per dimension).
That is 6.4× smaller than a 3,072 B raw f32 vector, but substantially larger
than a default 16 B PQ code. RaBitQ occupies a different recall/computation
point; its extended-code distance paths include ARM NEON SIMD.
...
```
 ...
 (c)      Train/load quantizer (PQ / SQ / RQ)
             • sample the selected source fragments
             • use IVF residuals for L2/Cosine PQ
             • per-sub-vector k-means (for PQ)              lance-index/src/vector/pq/builder.rs
 (d)      Assign rows to centroids and encode their codes   IVF/quantizer transforms
 (e)      Shuffle encoded rows by partition                lance-index/src/vector/v3/shuffler.rs
 (f)      Read each partition's shuffled codes
...
| `nprobes` (query-time) | automatic; try fixed 20 for a baseline | ↑ | ↓ |
| `num_sub_vectors` (PQ) | 16 (if D=768) | ↑ (must divide D) | ↓ |
| `num_bits` (PQ) | 8 (4 also supported) | — | — |
| `num_bits` (RQ) | 5 (range 1–9) | ↑ | ↓ |
| `m` (HNSW) | 20 | ↑ | ↓ |
...
Fixed `nprobes` sets both the minimum and maximum probe counts. With automatic
probing, the initial count is heuristic and filtered searches can expand it;
v12 adds a metric-aware profile for a limited IVF_FLAT query shape. See
`03-index-on-disk-and-search.md` §4 for eligibility and tuning details.
