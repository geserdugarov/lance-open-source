...
Python (`python/`): run `make install` once per fresh worktree to create and
sync the environment. After setup, run Python repository commands through
`uv run`—never bare `python`/`pytest`/`maturin`—and run `make build` after Rust
changes.
...
- **Unsafe without invariants.** Every `unsafe { ... }` block needs a `// SAFETY:` comment naming the invariants the caller is upholding (alignment, aliasing, lifetime, init state). `ptr::read` on a network/Arrow buffer without an alignment guarantee is UB even when the test passes on x86.
- **Cancel-safety in async.** A future used in `tokio::select!`, `timeout`, or a task set can be dropped mid-`.await`; cancellation between "write" and "ack" can duplicate work on retry. Analyze those call sites explicitly, and document or isolate a non-cancel-safe critical section when cancellation would leave externally visible partial state. Do not add boilerplate annotations to unrelated async functions.
...
Before committing Rust changes, run the Rust gates in the Pre-merge checklist above.
